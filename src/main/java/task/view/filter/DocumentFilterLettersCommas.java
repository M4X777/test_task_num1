...
public class DocumentFilterLettersCommas extends DocumentFilter {
    @Override
    ...
    public void replace(DocumentFilter.FilterBypass fp, int offset, int length, String stringToFilter, AttributeSet aset) throws BadLocationException {
        int len = stringToFilter.length();
        if (Character.isLetter(stringToFilter.charAt(len - 1)) || stringToFilter.charAt(len - 1) == ' ' || stringToFilter.charAt(len - 1) == ',')
